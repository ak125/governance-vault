...
Deux mémoires utilisateur ajoutées à `MEMORY.md` (scope DEV scratch, non-canonisées
intentionnellement — patterns de discipline opérationnelle pas gouvernance) :

- `feedback_prefer_mv_over_cp_for_relocation` — relocalisation fichier =
  `mv` atomique, jamais `cp` seul (laisse duplication).
- `feedback_strip_env_vars_python` — `.strip()` env vars au point de
