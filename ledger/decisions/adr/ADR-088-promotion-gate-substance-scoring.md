...
related_adr: ["ADR-033", "ADR-059", "ADR-083", "ADR-086"]
...
> et **ADR-086** (contrat de contenu structuré / engineBlock). Référence **ADR-040** (coverage-map),
...
avant de scorer A. ⚠️ **Backing vault à établir** : la coverage-map existe comme **schéma wiki** mais n'a **pas d'ADR
vault dédié** vérifié (ni ADR-040 = *seo-roles-canon-ts-side-only*, ni ADR-048 = *canon-enforcement-coverage audit*).
Cet ADR-088 doit soit **établir** ce backing, soit le **référencer une fois créé** — à trancher par l'owner.
...
| Dim Sources | agrégat `source_refs` (fiche) | **coverage-map, par claim** (ADR-040) |
...
2. **Backing vault de la coverage-map** : à établir (ni ADR-040 = *seo-roles*, ni ADR-048 = *audit canon*).
   Soit cet ADR-088 l'établit (ajouter une §), soit un ADR dédié — voir §C.
