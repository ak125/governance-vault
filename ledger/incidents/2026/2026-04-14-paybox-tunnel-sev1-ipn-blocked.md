...
id: INC-2026-002
type: incident
title: Paybox payment tunnel — 25 jours commandes non-confirmées
date: 2026-04-14
...
# INC-2026-002: Paybox payment tunnel — 25 jours commandes non-confirmées
...
| 2026-03-20 09:59 | Premier cliff-victim (ORD-1774000750441-805, 34.87 €) |
...
- **Utilisateurs affectés** : 4 clients distincts + 2 self-tests + 1 spam probable
...
> `paybox-callback-gate.service.ts` ligne 210-215 incluait `errorCode.ok` dans `allCriticalChecksOk`. Confusion entre *check sécurité* (anti-spoofing/replay/tampering) et *info business* (statut bancaire). Les refus bancaires signés valides par Paybox étaient rejetés.
...
### Bug #2 — Gate code (`paybox-callback-gate.service.ts`)
...
### Bug #3 — RPC SQL (migration `20260417_fix_mark_order_paid_atomic_type_error.sql`)
...
6. **Gouffre `.local/` → vault canonique** : ce post-mortem a été initialement écrit dans `/opt/automecanik/app/.local/governance-vault/` et n'était donc pas visible sur GitHub. Découvert 4 jours après. Origine de [[ADR-015-vault-single-source-of-truth|ADR-015]].

## Actions Correctives

### Préventions actives (déployées)

- [x] **PREV-1** — Cron 15min alerting email Gmail OAuth2 (`scripts/monitoring/check-payment-tunnel.sh`) — **Owner** : @automecanik.seo — **Deadline** : 2026-04-18 — **Status** : FAIT
- [x] **PREV-4 Phase 1** — Caddy `roll_keep 10→2000` + `roll_keep_for 720h` → rétention 30j — **Owner** : @automecanik.seo — **Deadline** : 2026-04-18 — **Status** : FAIT
- [x] **M1** — Sanitize logs `paybox.service.ts` (PII + merchant IDs) — **Owner** : @automecanik.seo — **Deadline** : 2026-04-18 — **Status** : FAIT
- [x] **M2** — Bug #2 Gate `errorCode` fix (13 tests) — **Owner** : @automecanik.seo — **Deadline** : 2026-04-18 — **Status** : FAIT
...
- [ ] **Runbook** `payments-tunnel-debug.md` — checklist reproductible — **Owner** : @tech — **Deadline** : 2026-04-30
...
- Incident précédent similaire : [[2026-02-03-paybox-orderid-format|INC-2026-01-30]]
- ADR gouvernance issue de cet incident : [[ADR-015-vault-single-source-of-truth]]
- ADR sécurité connexe : [[ADR-014-remove-paybox-callback-test]]
- MOC : [[MOC-Incidents]]
