#!/usr/bin/env bash
# new-incident.sh — Scaffold un nouveau post-mortem incident
#
# Usage:
#   ./scripts/new-incident.sh <severity> <slug> [id]
#
# Exemples:
#   ./scripts/new-incident.sh critical paybox-tunnel
#   ./scripts/new-incident.sh high redis-sessions-loss INC-2026-003
#
# Effets:
#   1. Crée fichier 01-incidents/YYYY/YYYY-MM-DD-<slug>.md depuis le template
...
TODAY="$(date +%Y-%m-%d)"
YEAR="$(date +%Y)"
TARGET_DIR="01-incidents/$YEAR"
TEMPLATE="01-incidents/_templates/incident-template.md"
...
if [ -z "$CUSTOM_ID" ]; then
  LAST_NUM="$(grep -rEho 'id: INC-'"$YEAR"'-[0-9]+' 01-incidents/ 2>/dev/null \
    | sed -E 's/.*-([0-9]+)$/\1/' \
...
# Incident: [Titre court — à remplir]
...
echo "  2. Lier depuis 00-index/MOC-Incidents.md (R-Vault-02: zéro orphelin)"
echo "  3. Valider: ./scripts/check-orphans.sh"
