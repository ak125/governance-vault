...
| ADR-052 | Hoist `handoff_targets` du canon markdown vers `@repo/seo-roles/handoff-graph.ts` (mirror typé) + amendement canon R6 → R1 + backend dérivation `isLinkAllowed` / `isRenderableLinkAllowed` + observabilité `seo_handoff_filtered` | Proposed | 2026-05-08 | [[ADR-052-handoff-graph-canon]] |
