...
- [[cwv-rum-pipeline-recovery]] - Runbook récupération pipeline **RUM** CWV interne (`__seo_cwv_raw` → `__seo_cwv_hourly` → `__seo_cwv_daily_rum`), orchestré **pg_cron** ([[ADR-045-seo-monitoring-cron-v0]] amendement 2026-06-26). Distinct du CrUX [[cwv-alert-response]] (field Google, latence 7-10j) et du lab `__seo_cwv_daily`. Diagnostic : 2 jobs cron + `cron.job_run_details` (`log_run=on`), anti-join couverture raw→hourly, probe Bull résiduel `bull-repeatable-drift.ts`, partitions. Réparation : mesurer+journaliser la perte AVANT tout backfill (raw TTL ~48h ; perte passé TTL **non reconstructible**), backfill hourly AVANT daily (idempotent UPSERT), purge repeatable Bull, résolution manuelle d'alerte coincée, rollback `.down` ciblé. Notes latentes : clé `ON CONFLICT` sans `priority_tier` (last-wins silencieux), `fetched_at`=dernière recompute (pas première vue). Preuve rétablissement = `get_cwv_dashboard` lignes fraîches J-1. Incident `web-vitals-attribution-unstable` ; monorepo #1165/#1166.
