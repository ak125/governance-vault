...
- [[sprint-perf-bundle-lessons-20260508]] - Sprint perf bundle 7 PRs (vault #211 + monorepo #387/391/393/394/395/396/397) — refus PR #382 recalibration arbitraire 1250→3500 KB. 7 consignes méthodo signal-proven : (1) matrice raw/gzipped/per-page non-négociable ; (2) vérifier plancher technique avant d'optimiser (admin exclu prod, lucide tree-shake déjà optimal, Remix race condition vendor split) ; (3) 9 hypothèses vendor (devtools, dups React, react-dom-server fuite, jsx-dev-runtime, PropTypes, html-parser, framer-motion, dups utils, manifest bloat) ; (4) audit Remix payloads obligatoire (`__remixContext` + loaders dups) ; (5) branchement décisionnel A/B ADR-051 ; (6) stacked PR cascade max 3 niveaux + gotchas auto-merge GraphQL ; (7) limitation visualizer dual-build. Mesure CI réelle dépassement = +9 KB (pas +1.85 MB de la confusion initiale). PR-4 lazy modal (-13.8 KB) > dépassement → débloque mécaniquement #381/#359/#365.
